...
When we create a products index, there would be multiple segments in form of .segment files are created for this
...
> Analogy : 
  ...
  - DELETE /_products/_doc/10 is run to delete the entry of product-10 stores it as (.del files - in older version and .cfs extension - in newer versions) 
...
> The older version of Lucene used to have .del physical files, but in modern Lucene versions, these have been replaced. Deletion information is now typically tracked within embedded compound files (with the .cfs extension, I believe), or in related metadata files.
>  For deep-dive technical questions, the Elasticsearch community forum is an excellent place to ask https://discuss.elastic.co/latest.
